                                ...
                                <div key={post.slug} className={cn("overflow-hidden bg-white dark:bg-gray-800 rounded shadow")}>
                                    <div className="p-5">
                                        <div className="relative">
                                            <Link href={`/${convertToSlug(post.category)}/${convertToSlug(post.slug)}`} title={post.title} className="block aspect-w-4 aspect-h-3">
                                                <img className="object-cover w-full h-full" src={post.featured_image + "?v=" + post.slug} alt="" />
                                            </Link>
                                            <div className="absolute top-4 left-4">
                                                <Link href={`/${convertToSlug(post.category)}`}>
                                                    <span
                                                        className={cn(
                                        ...
                                        <p className="mt-5 text-2xl font-semibold">
                                            <Link
                                                href={`/${convertToSlug(post.category)}/${convertToSlug(post.slug)}`}
                                                title={post.title}
                                                className={cn("text-black dark:text-white hover:text-blue-600 dark:hover:text-blue-400")}
                                            >
                                                {post.title}
                                            </Link>
                                        </p>
